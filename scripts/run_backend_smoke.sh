...
latest_run="$(find "$repo_root/results/aggregated" -maxdepth 1 -mindepth 1 -type d -name '*paper_smoke*' | sort | tail -n 1)"

if [[ -z "${latest_run:-}" ]]; then
  echo "[PICASSO] ERROR: no aggregated smoke run found" >&2
  exit 1
fi
